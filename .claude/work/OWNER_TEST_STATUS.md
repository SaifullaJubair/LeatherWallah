...
|-----------|-------------|-------------|
| **Session 47 batch-3 — order-status dropdown (A2.2/A2.3) + stockless draft (A1.1) + F1.1 flash half. (1) A2.2/A2.3: reachable order-status `<select>` in ViewAllOrderInfo (forward + cancel/return w/ reason; courier-locked orders blocked) + BE `updateOrderServices` allowed-transition guard. (2) A1.1: simple-product quantity now required only to PUBLISH (Save Draft works stockless). (3) F1.1 flash: FE flash "fixed" = absolute price + flash "percent" off post-discount base — both match BE resolver (BE charge untouched, FE display corrected).** | ✅ Code done s47 (BE `794cd5c`; Admin `4d1ca2a`+`c24a87b`+`4312533`; FE `e20a637`+`da735e9`; `dev`). BE tsc EXIT 0 · Admin Vite EXIT 0 · helper.js node --check OK. code-reviewer pass: A2.2/A2.3/A1.1 clean; flash percent-base + return-lock + const-hoist fixes applied. | 🔵 STATIC-VERIFIED — owner browser test: **(A2.2)** open an order → "Change status →" dropdown shows valid next steps → advance pending→confirmed (SMS fires) →processing→shipped→delivered→completed; cancel/return ask reason; courier-sent order blocks cancel/return here. Try a crafted/curl jump pending→completed → 400. **(A1.1)** add simple product, leave Quantity blank → Save Draft works; try Publish with blank Quantity → blocked. **(flash)** flash "fixed ৳300" product on ৳500 base → PDP+cart show ৳300 (was ৳200); flash "percent" on a product that also has a discount → shown == charged. |
| **Session 48 — Playwright BROWSER test pass (price/variation/cart). Live-verified the Session-47 FE F1.1 campaign-cart enrich AND found a follow-on BLOCKER it exposed: `CART-CAMP`. The enrich made campaign reach the cart, but `helper.js` `productPrice()` campaign branch used `product_discount_price` as base → cart showed e.g. ৳450 while PDP + BE recompute charge ৳750 (shown<charged MONEY bug). FIXED `helper.js` campaign base → regular `product_price` (mirrors flash branch + `product.price.resolver.ts` + `order.recompute.ts`). Also reproduced+root-caused F4.1 reviews-carousel 400 (still a ticket).** | ✅ Code: `FruitSnacksFrontend/src/utils/helper.js` campaign base fix (FE `dev`, NOT committed — owner testing first). `node --check` OK. Test-data helper added: `FruitSnacksBackend/src/scripts/seed-promotions.ts` + `npm run seed:promo` (flash+campaign+offer+2 warehouse+2 supplier; idempotent; `--force`; detects own rows by `(Demo)` title marker — these schemas have no `is_demo`). Full report: `.claude/work/playwright-tests/RESULTS_2026-06-19_session.md`. | ✅ Claude browser-verified: flash ৳840, campaign PDP ৳1360/৳750, **campaign cart ৳750 after fix (was ৳450)**, variation selector+URL+price. 🔵 owner re-confirm: add campaign product (vintage-saddle) to cart → line=৳750 not ৳450 → place order → charged=cart. ❌ F4.1 open ticket: home reviews carousel never renders — `ReviewsCarousel.jsx` calls `GET /review?…review_rating=5&has_photo=true` which routes to `findUserReview` (needs publisher id) → 400. Needs a real public featured-reviews endpoint (or repoint to one). |
| **Session 47 — FE F4.1/F3.1 + F4.2 — home reviews carousel never rendered (auto_featured mode hit `GET /review?...review_rating=5&has_photo=true` → routed to `findUserReview` → 400 → empty → section returned null). NEW public endpoint `GET /review/featured` (active + `review_ratting>=5` + `review_image` present, product name/slug populated, `review_user_id` stripped, mounted BEFORE the `:review_product_id` wildcard). Carousel auto query repointed to it. F4.2: card now reads real DB field names `review_ratting`/`review_image`/`review_product_id.product_name` (were `review_rating`/`review_photo`/`product_id` → 0-star/no-photo/no-product even in manual mode).** | ✅ Code done s47 (BE `4a1ce8a` + FE `424d961`, `dev`). BE tsc EXIT 0. Route ordering verified (before wildcard). | 🔵 STATIC-VERIFIED — needs owner browser test: home page → reviews carousel section now RENDERS (was always blank) IF there are active 5-star reviews WITH a photo (seed some via Admin → Seed Reviews with image + rating 5, or `npm run seed:promo` if it seeds reviews). Each card shows ★★★★★ + photo + reviewer name + product name. Empty state (no qualifying reviews) → section hidden (correct, graceful). Manual-pick mode (settings) also now shows correct stars/photo. |
| **Session 47 — FE F1.1 (partial: CAMPAIGN half) — cart now attaches `campaign_details` so (a) cart UI shows campaign price + (b) checkout sends a REAL `campaign_id` (was always null → campaign discount silently lost). Batched campaign lookup ($in active campaigns) added to `findCartProductServices` (BE `product.services.ts`), same shape as PDP/strip path. `staleTime: Infinity → 60_000` on cart fetch (`AddToCart.jsx`) to cap shown-vs-charged window for expiring promos.** | ✅ Code done session 47 (BE + FE `dev`, **NOT committed yet at time of test-row / owner testing first**). BE tsc EXIT 0 (dev tree). `GET /cart` path confirmed NOT a price source (only feeds Redux qty state) so single-path enrich suffices. **FLASH HALF DEFERRED** (edge-audit B1/B2): flash "fixed"=absolute (BE resolver.ts:112) vs subtraction (FE helper.js:102), + flash percent base differs on variations → aligning touches core price resolver, separate ticket. Flash stays known "regular shown / flash charged" (under-charge, buyer-favourable). Committed BE `5e97db7` + FE `09fdb86` (`dev`). | ✅ CLAUDE BROWSER-VERIFIED (s48, Playwright): campaign now reaches the cart (was lost). Found+fixed a follow-on money bug during this test — `helper.js` campaign base was `product_discount_price` → cart showed ৳450 but BE charged ৳750 (shown<charged); corrected to regular `product_price` → cart now ৳750 == BE charge ✅ (folded into FE `09fdb86`). PDP campaign %/fixed also verified (৳1360 / ৳750). Owner can still do a full place-order to confirm `campaign_id` + `discount_source:"campaign"` land in the DB order. |
| **Session 47 — Admin deep-audit HIGH fixes (A2.1 + A1.1): A2.1 `/pathao-order` page repointed from `/order/dashboard?order_status=shipped` (all couriers, shipped only) → `/order/pathao` (real Pathao-only endpoint, `getPathaoOrders`, same `{data,totalData}` shape). A1.1 `product_buying_price` RHF-`required` dropped on simple-product create (`StepOnePrice.jsx`) so Save Draft works without buying price (backend treats it optional).** | ✅ Code done session 47 (Admin `dev`, **NOT committed — owner testing first**). Admin Vite build EXIT 0. Backend `/order/pathao` route confirmed (gated `order_show`, supports page/limit/searchTerm). **DEFERRED sub-item:** `product_quantity` still RHF-required + `validateSimpleNumeric` requires qty on BOTH draft & publish → stockless draft still blocked. Left as-is (owner decision: do you want stockless drafts? + needs publish-flag threaded into StepOnePrice/validateSimpleNumeric). | ✅ CLAUDE BROWSER-VERIFIED (s48, Playwright): **(A2.1)** `/pathao-order` now calls `GET /order/pathao?...` → 200 (was `/order/dashboard?order_status=shipped`); list empty (no pathao orders seeded) but endpoint repoint correct. **(A1.1)** `/product/product-create`: "Product Buying Price" has NO required asterisk + input `required:false`; required = Name*/Category*/Description*/Product Price*/Product Quantity*; "Save Draft" button present. ⚠️ Product Quantity STILL required (deferred, as noted). Owner can still do a full place-order/save round-trip to confirm. |
| **Session 47 — Admin deep-audit BLOCKER fixes (A4.1 + A4.2/A4.3): warehouse FE guards repointed off ghost flags (`setting_show`/`setting_update`) → `site_setting_update` in WarehousePage/SideNavBar/WarehouseTable (6 guards); permissionData.js uncommented Question + Offer + Campaign + Slider blocks (specification left commented — module retired)** | ✅ Code done session 47 (Admin `dev`, **NOT committed yet — owner testing first**). Admin Vite build EXIT 0 (3974 modules). Ghost-flag scan clean. Backend warehouse route already fixed on `dev` `bf4cc0b` (B1). Flags verified present in `role.model.ts` + gated by real routes + emitted by CreateStaffRole/UpDateStaffRole | ✅ CLAUDE BROWSER-VERIFIED (s48, Playwright): **(1) Warehouse** "Warehouses" visible in sidebar → `/warehouse` opens → "Create Warehouse" button present + 2 seeded rows show. **(2) Custom role** `/create-staff-role` shows Question + Offer(C/U/S/D) + Campaign(C/U/S/D) + Slider(C/U/S/D) + Flash blocks (92 perms total) — previously commented-out → ungrantable. Owner can still tick+save a role + log in as that staff to confirm end-to-end. **NOTE:** offer_* also gates Flash Sale (one block grants both) |
