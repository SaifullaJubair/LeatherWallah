...
- ✅ **SEO audit (s39)** — fixed robots↔sitemap↔pageSeo conflict: `/shop` + `/offer` were blocked (now indexable), 5 legacy listing routes set noIndex (they 301→/shop). Deleted dead pageSeoManagement + console.log. meta_keywords confirmed already wired.
- ✅ **Dynamic-PDP chain audit (s39)** — traced product-create → theme assign → page-content (nutrition/benefits/use_cases/faqs/badge) → floating images → themed render. Whole chain consistent across Admin↔BE↔FE; every content type has a dedicated PDP section. Fallbacks safe (no theme → mergeTheme default green; per-product color override intentionally removed per the black-PDP fix). Removed the dead `/products-original` legacy PDP route (new PDP uses a separate component set).
