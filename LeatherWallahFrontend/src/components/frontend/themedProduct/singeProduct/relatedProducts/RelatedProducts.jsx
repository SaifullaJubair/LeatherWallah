...
const RelatedProducts = ({ product_slug }) => {
  ...
  return (
    <section className="py-10 md:py-12" style={{ background: "var(--section-bg)" }}>
     <div className="max-w-6xl mx-auto px-4">
      <div data-related-v2 style={{ background: "#22c55e", color: "#fff", padding: "4px 8px", fontSize: 11, marginBottom: 8, borderRadius: 4 }}>
        DEBUG: RelatedProducts v2 (count={relatedProduct?.data?.length}, showing {Math.min(4, relatedProduct?.data?.length || 0)})
      </div>
      ...
      {relatedProduct?.data?.slice(0, 4).map((product, index) => {
