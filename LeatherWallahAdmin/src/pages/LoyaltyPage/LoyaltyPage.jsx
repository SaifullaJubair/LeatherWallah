/**
 * Admin Loyalty viewer + adjust (Phase G3, A3b).
 *
 * Two panes:
 *   - Top: user picker (react-select, server-searched against /user) + balance card.
 *   - Bottom: ledger table (newest first) + adjust form (delta + reason).
 *
 * Backend: GET /loyalty/history/admin?user_id=…  (verifyToken user_show)
 *          POST /loyalty/adjust {user_id, delta, reason}  (verifyToken user_update)
 */
...
const LoyaltyPage = () => {
  ...
  const {
    ...
    queryKey: [
      `/api/v1/loyalty/history/admin?user_id=${selectedUser?.value}&page=${page}&limit=${limit}`,
    ],
    queryFn: async () => {
      if (!selectedUser?.value) return null;
      const res = await fetch(
        `${BASE_URL}/loyalty/history/admin?user_id=${selectedUser.value}&page=${page}&limit=${limit}`,
        { credentials: "include" },
      );
      if (!res.ok) {
  ...
  const handleAdjust = async () => {
    ...
    if (!Number.isFinite(n) || n === 0) {
      toast.error("Delta must be a non-zero number (positive = credit, negative = debit)");
      return;
    }
    setAdjusting(true);
    try {
      const res = await fetch(`${BASE_URL}/loyalty/adjust`, {
        method: "POST",
        credentials: "include",
      ...
      if (result?.statusCode === 200 && result?.success === true) {
        toast.success(
          `Loyalty ${n > 0 ? "credited" : "debited"}. New balance: ${result?.data?.balance ?? "?"}`,
          { autoClose: 1500 },
        );
        setDelta("");
        setReason("");
    ...
    <>
      {adminUser?.role_id?.user_show === true && (
        <div className="bg-white rounded-lg py-6 px-4 shadow space-y-6">
          <div>
            <h1 className="text-2xl">Loyalty Points</h1>
            <p className="text-xs text-gray-400 mt-1">
              Pick a customer, view their points ledger, and (with{" "}
              <code>user_update</code> permission) manually credit/debit points.
            </p>
          </div>

          {/* User picker + balance card */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-gradient-to-br from-rose-50 to-pink-50 border border-rose-100 rounded-lg p-4">
              <p className="text-xs font-semibold uppercase text-gray-500 mb-1">
                Current balance
              </p>
              <p className="text-2xl font-bold text-rose-700">
                {selectedUser ? balance : "—"}{" "}
                <span className="text-sm font-medium text-rose-500">pts</span>
              </p>
              {selectedUser && (
          ...
          {selectedUser && canAdjust && (
            <div className="border border-rose-100 bg-rose-50/40 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                ...
                <div className="md:col-span-7">
                  ...
                  <input
                    ...
                    placeholder="e.g. compensation for delayed order #INV123"
                    className="w-full px-3 py-2 border border-gray-300 rounded text-sm"
                  />
                </div>
                <div className="md:col-span-2">
                  {adjusting ? (
                    <div className="w-full py-2 flex items-center justify-center bg-rose-600 text-white rounded">
                      <MiniSpinner />
                    </div>
                  ) : (
                    <button
                      ...
                      className="w-full py-2 bg-rose-600 hover:bg-rose-700 text-white text-sm font-medium rounded"
                    >
                      Apply
                    </button>
                  )}
