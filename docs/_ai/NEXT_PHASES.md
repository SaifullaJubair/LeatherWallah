...
- **FE F4.1/F3.1** — home ReviewsCarousel default `auto_featured` hits `GET /review?review_rating=5&has_photo=true` → 400 → section never renders. Needs public featured-reviews endpoint (BE) + field-name fix (F4.2). **(still OPEN — next)**
