...
Deferred follow-ups: ~~`PATCH /product/images` to handle main_video swap/remove~~ ✅ DONE (verified s52 — `patchProductImagesServices` has `swap_video` + `remove_video` modes, full upload + old-key delete); wrap product CRUD in mongoose transactions (**PARTIAL: `postProduct`/create IS transactional; `updateProduct`/`patchProductQuick`/`deleteAProductInfo` are NOT — hardening, not a live bug; needs edge-audit before touching the multi-doc update path**); self-heal `attribute_id` by name lookup.
...
**🔴 BLOCKERs (4):**
- **FE F1.1 ✅ FULLY DONE (s47)** CAMPAIGN half (BE `5e97db7` + FE `09fdb86`): cart enriches `campaign_details` → campaign price shown + real `campaign_id` to checkout; campaign base fixed to regular `product_price`; cart `staleTime` 60s. **FLASH half (FE `e20a637` + `da735e9`):** FE now matches the BE resolver — flash "fixed" = absolute price (was subtraction → ৳200 shown vs ৳300 charged) and flash "percent" applies off the post-discount base (was undiscounted → mismatch when discount+flash coexist). **BE pricing authority untouched → no live flash sale's charge changed; only FE display corrected.** (campaign "fixed" stays a subtraction via applyCampaign — intentionally different.)
...
- **Admin A2.2/A2.3 ✅ DONE (s47, owner chose: wire it)** Admin `4d1ca2a`+`4312533` + BE `794cd5c`. Reachable forward/terminal status dropdown in `ViewAllOrderInfo` (forward steps PATCH; cancel/return prompt a reason; courier-locked orders can't be cancelled/returned here). BE `updateOrderServices` now has an allowed-transition guard (rejects bad jumps; courier webhooks use direct updateOne so unaffected). FE option map ↔ BE guard kept in sync.
...
**🟠 HIGH — ALL DONE as of s52:** ~~FE F4.2 (carousel wrong field names)~~ ✅ DONE s47 (FE `424d961`, with F4.1), ~~F3.3 (/shop sort page-local not catalog)~~ ✅ DONE s52 (BE `1af226a` server-side `$sort` + FE `2ec6802` `&sort=`+queryKey; Playwright+API verified), **F3.4 ✅ DONE (s47, BE `4578e9e`)** the 3 unwired default-ON sections (trust_strip/feature_categories/offers_block) shipped OFF in HOME_SECTION_DEFAULTS (no FE component yet → were blank gaps at fresh-clone home top; owner re-enables from Home Layout when built; existing DBs untouched), ~~F1.3 (COD phone 2 formats)~~ ✅ DONE s52 (BE `1af226a` normalize every order path + FE `2ec6802` chokepoints; DB read-back + abandoned-cart recovery verified), ~~F1.2 (cart qty additive merge)~~ ✅ DONE s52 (FE `2ec6802` clampQty; 7/7 reducer assertions); **Admin A4.2/A4.3 ✅ DONE (s47, `d3c6c88`)** permissionData Question/Offer/Campaign/Slider uncommented (Flash gated by offer_*); **A1.1 ✅ FULLY DONE (s47)** buying-price required dropped (`d3c6c88`) + stockless draft: quantity now required only to PUBLISH, optional on Save Draft (`c24a87b`, owner chose yes — build product now, set stock later); **A2.1 ✅ DONE (s47, `d3c6c88`)** /pathao-order repointed to /order/pathao.
